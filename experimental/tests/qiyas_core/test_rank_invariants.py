...
class TestRankInvariants:
    ...
    def test_output_rank_never_exceeds_rule_ceiling(self):
        ...
        candidate = make_candidate_with_rank(EvidenceRank.FORM)
        assert_rank_ceiling(candidate, EvidenceRank.FORM)
        assert_rank_ceiling(candidate, EvidenceRank.QIYAS)  # Higher ceiling is OK

    def test_rank_ceiling_violation_detected(self):
        """Helper detects when rank exceeds ceiling"""
        candidate = make_candidate_with_rank(EvidenceRank.QIYAS)
    ...
    def test_rank_is_minimum_of_all_inputs(self):
        ...
        rule = make_minimal_rule(rank_ceiling=EvidenceRank.QIYAS)
        asl = make_unicode_node(rank=EvidenceRank.FORM)
        ...
        assert_rank_is_minimum(
            ...
            asl_rank=asl.rank,
            far_rank=far.rank,
            evidence_rank=EvidenceRank.QIYAS,
        )

    def test_low_asl_rank_limits_output(self):
        """Low asl rank limits output rank"""
        rule = make_minimal_rule(rank_ceiling=EvidenceRank.QIYAS)
        asl = make_unicode_node(rank=EvidenceRank.FORM)  # Low rank
        far = make_haraka_node(rank=EvidenceRank.QIYAS)  # High rank
    ...
    def test_low_far_rank_limits_output(self):
        ...
        rule = make_minimal_rule(rank_ceiling=EvidenceRank.QIYAS)
        asl = make_unicode_node(rank=EvidenceRank.QIYAS)  # High rank
        far = make_haraka_node(rank=EvidenceRank.FORM)  # Low rank
    ...
    def test_low_rule_ceiling_limits_output(self):
        ...
        rule = make_minimal_rule(rank_ceiling=EvidenceRank.FORM)  # Low ceiling
        asl = make_unicode_node(rank=EvidenceRank.QIYAS)
        far = make_haraka_node(rank=EvidenceRank.QIYAS)
    ...
    def test_all_high_ranks_produces_high_output(self):
        ...
        candidate = result.candidates[0]
        assert candidate.rank == EvidenceRank.QIYAS
    ...
    def test_rank_minimum_assertion_detects_wrong_rank(self):
        ...
        with pytest.raises(AssertionError, match="should be"):
            assert_rank_is_minimum(
                candidate,
                rule_ceiling=EvidenceRank.QIYAS,
                asl_rank=EvidenceRank.FORM,  # Minimum
                far_rank=EvidenceRank.QIYAS,
                evidence_rank=EvidenceRank.QIYAS,
            )
    ...
    def test_form_rank_ceiling_respected(self):
        ...
        assert_rank_ceiling(candidate, EvidenceRank.QIYAS)
